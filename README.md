...
بدون نصب، بدون حساب کاربری، کاملاً رایگان و سمت کاربر (client-side). هیچ داده‌ای به سرور فرستاده نمی‌شود. طرح‌ها (پروژه‌ها) به‌صورت **event-sourced** در `IndexedDB` مرورگر ذخیره می‌شوند؛ تنظیمات و فونت‌های دلخواه در `localStorage` می‌مانند.
